...
require_once('/var/www/html/xhgui/vendor/autoload.php');
...
register_shutdown_function(function ()
{
    ...
    if (extension_loaded('uprofiler')) {
        ...
        $guzzleProfile = json_encode($data['profile']);
    } else if (extension_loaded('tideways')) {
    ...
    $guzzleData['meta'] = array(
        'url' => $uri,
        'SERVER' => json_encode($_SERVER),
        'get' => $_GET,
        'env' => $_ENV,
        'simple_url' => Xhgui_Util::simpleUrl($uri),
        'request_ts' => date('Y-m-d H:i:s', $time),
        'request_ts_micro' => date('Y-m-d H:i:s', $requestTimeFloat[0]),
        'request_date' => date('Y-m-d', $time)
    );

    $guzzleData['profile'] = $guzzleProfile;

    try {
        $guzzle = new \GuzzleHttp\Client();

        $guzzle->post('http://192.168.209.128:3000/firebase', [
            'body' => $guzzleData
        ]);
